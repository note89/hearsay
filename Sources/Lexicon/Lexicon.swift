...
public enum LexiconEntry: Equatable {
    case term(String)
...
public struct Lexicon: Equatable {
    ...
    public static func load(from url: URL) -> Lexicon {
        ...
        for rawLine in content.split(separator: "\n") {
            ...
            if let arrow = line.range(of: "->") {
                ...
                if !from.isEmpty && !to.isEmpty { rewrites.append((from: from, to: to)) }
            } else {
                terms.append(line)
            }
        }
        return Lexicon(terms: terms, rewrites: rewrites)
    }
    ...
    public static func ensureFile(at url: URL) -> URL {
        if !FileManager.default.fileExists(atPath: url.path) {
            ...
            FileManager.default.createFile(atPath: url.path, contents: template.data(using: .utf8), attributes: [.posixPermissions: 0o600])
        }
        return url
    }
